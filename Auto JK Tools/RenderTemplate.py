...
def changeBackground(color):
    ...
    bpy.data.objects['BackgroundColor'].select_set(True)
    bpy.context.view_layer.objects.active = bpy.data.objects['BackgroundColor']
    ...
    if color == 'Red':
        bpy.ops.image.open(filepath = addonPath + '\\Auto Jk Tools\\Background Colors\\Background_Red.png')
        bgImage = bpy.data.images['Background_Red.png']
    elif color == 'Orange':
        bpy.ops.image.open(filepath = addonPath + '\\Auto Jk Tools\\Background Colors\\Background_Orange.png')
        bgImage = bpy.data.images['Background_Orange.png']
    elif color == 'Blue':
        bpy.ops.image.open(filepath = addonPath + '\\Auto Jk Tools\\Background Colors\\Background_Blue.png')
        bgImage = bpy.data.images['Background_Blue.png']
    elif color == 'Purple':
        bpy.ops.image.open(filepath = addonPath + '\\Auto Jk Tools\\Background Colors\\Background_Purple.png')
    ...
    #change ground color
    ground_red = [0.21404114365577698, 0.07081720978021622, 0.053595349192619324, 1.0]
    ground_orange = [0.25295013189315796, 0.14132976531982422, 0.07005347311496735, 1.0]
    ground_blue = [0.06383193284273148, 0.10823961347341537, 0.19599416851997375, 1.0]
    ground_purple = [0.09730064123868942, 0.0911921039223671, 0.23302194476127625, 1.0]

    bpy.data.objects['BackgroundColor'].select_set(False)
    bpy.data.objects['Ground'].select_set(True)
    bpy.context.view_layer.objects.active = bpy.data.objects['Ground']

    mat=bpy.context.active_object.active_material
    tree=mat.node_tree.nodes
    groundNode = tree.get("GroundColor")
    if color == 'Red':
        groundNode.outputs[0].default_value = ground_red
    elif color == 'Orange':
        groundNode.outputs[0].default_value = ground_orange
    elif color == 'Blue':
        groundNode.outputs[0].default_value = ground_blue
    elif color == 'Purple':
        groundNode.outputs[0].default_value = ground_purple
...
def checkPSFinished(path):
    for (root, dir, files) in os.walk(path, topdown = True):
        for f in files:
            fileName = os.path.basename(f)
            if fileName == 'RenderedImage.psd':
                return True
    return False
...
class REAL_OT_AutoRenderer(Operator):
    ...
    def execute(self, context):
        ...
        os.system("start Photoshop.exe " + bpy.utils.user_resource('SCRIPTS', "addons") + "\\Auto JK Tools\\Combine2.jsx")
        
        while(True):
            time.sleep(5)
            finished = checkPSFinished(imgPath)
            print(finished)
            if finished:
                break
        ...
        for (root, dir, files) in os.walk(imgPath, topdown = True):
            for f in files:
                fileName = os.path.basename(f)
                if fileName == 'Scene_Background.png0000.png' or fileName == 'RenderedImage.psd':
...
class Appen_OT_File(Operator):
    ...
    def execute(self, context):
        ...
        file_path = bpy.utils.user_resource('SCRIPTS', "addons") + '\\Auto JK Tools\\3DCharacterRenerTemplate.blend'
        scene_path = 'Scene'
        ...
        bpy.ops.wm.append(
            filepath = os.path.join(file_path, 'Material', 'Background'),
            directory = os.path.join(file_path, 'Material'),
            filename='Background'
        )
        bpy.context.window.scene = bpy.data.scenes['Background']
        bpy.data.objects['Background'].select_set(True)
        bpy.context.active_object.active_material = bpy.data.materials['BackGround']
...
class REAL_PT_AutoPanel(Panel):
    ...
    def draw(self, context):
        ...
        row2.scale_y = 1
        row2.operator('operator.appendtemplate', text="导入Blender模板", icon = 'BLENDER')
        
        row1 = layout.row(align=True)
        row1.scale_y = 1
        row1.operator("wm.url_open", text="Help", icon = 'QUESTION').url = "https://iwiki.woa.com/pages/viewpage.action?pageId=807726005"
